ma.pages.barrier.wto = (function () {
    function setupAttachments(
        selector,
        fieldName,
        attachText = "Attach document"
    ) {
        if (
            !(
                ma.components.FileUpload &&
                ma.components.Attachments &&
                ma.components.AttachmentForm &&
                ma.components.TextArea &&
                jessie.hasFeatures(
                    "queryOne",
                    "cancelDefault",
                    "getElementData"
        ...
        try {
            attachments = new ma.components.Attachments(
                fileUpload,
                selector,
                fieldName
            );
            new ma.components.AttachmentForm(
                fileUpload,
                attachments,
                submitButton,
                false
