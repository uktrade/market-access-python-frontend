...
class EditBarrierTypesTestCase(MarketAccessTestCase):
    def test_edit_types_landing_page(self):
        ...
        response = self.client.get(
            reverse("barriers:edit_types", kwargs={"barrier_id": self.barrier["id"]})
        )
    ...
    def test_add_type_choices(self):
        ...
        response = self.client.get(
            reverse("barriers:add_type", kwargs={"barrier_id": self.barrier["id"]}),
        )
        assert response.status_code == HTTPStatus.OK
        assert "form" in response.context
        form = response.context["form"]
    ...
    def test_edit_types_confirmation_form(self):
        ...
        response = self.client.get(
            reverse(
                "barriers:edit_types_session", kwargs={"barrier_id": self.barrier["id"]}
            ),
        )
        assert response.status_code == HTTPStatus.OK
        form = response.context["form"]
    ...
    def test_edit_types_confirm(self, mock_patch):
        ...
        self.update_session(
            {
                "barrier_types": [
                    {"id": type_id, "title": "Title",} for type_id in new_types
                ],
            }
        )
