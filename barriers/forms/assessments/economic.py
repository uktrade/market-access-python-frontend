...
class TradeCategoryForm(forms.Form):
    ...
    def __init__(self, trade_categories, barrier=None, *args, **kwargs):
        ...
        self.fields["trade_category"].choices = [
            (key, value) for key, value in trade_categories.items()
        ]

    def save(self):
        client = MarketAccessAPIClient(self.token)
        client.barriers.patch(
            id=self.barrier.id, trade_category=self.cleaned_data["trade_category"]
        )
...
class EconomicAssessmentRatingForm(DocumentMixin, forms.Form):
    ...
    def __init__(
        self, ratings, barrier=None, economic_assessment=None, *args, **kwargs
    ):
        self.barrier = barrier
        self.economic_assessment = economic_assessment
        super().__init__(*args, **kwargs)
        self.fields["rating"].choices = [(key, value) for key, value in ratings.items()]
    ...
    def save(self):
        ...
        if self.economic_assessment:
            client.economic_assessments.patch(
                id=self.economic_assessment.id, **self.cleaned_data
            )
        elif self.barrier:
