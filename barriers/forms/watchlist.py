...
class SaveWatchlistForm(forms.Form):
    ...
    replace_or_new = forms.ChoiceField(
        ...
        error_messages={
            "required": (
                "Select to either create a new watch list or replace a current list"
            )
        },
    )
    replace_index = forms.ChoiceField(
        label="Which list would you like to replace?", required=False,
    )
    ...
    def __init__(self, watchlists, filters, *args, **kwargs):
        ...
        self.fields["replace_index"].choices = [
            (str(index), watchlist.name) for index, watchlist in enumerate(watchlists)
        ]
    ...
    def clean(self):
        ...
        if cleaned_data.get("replace_or_new") == self.REPLACE:
            if not cleaned_data["replace_index"]:
                self.add_error("replace_index", "Select which watch list to replace")

    def save(self):
        new_watchlist = Watchlist(name=self.cleaned_data["name"], filters=self.filters,)
        ...
        if self.has_to_create() or replace_or_new == self.NEW:
            ...
            self.watchlists = (
                self.watchlists[:index] + [new_watchlist] + self.watchlists[index + 1 :]
            )
