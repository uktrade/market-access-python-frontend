...
def test_add_a_barrier_early_exit(browser):
    ...
    draft_barriers = browser.find_by_css(
        '.my-draft-barriers .draft-barrier-item'
    )
...
def test_continue_draft_barrier(browser):
    ...
    assert sections[0].find_by_css(
        '.task-list__item__banner'
    ).value == "COMPLETED"
    assert sections[1].find_by_css(
        '.task-list__item__banner'
    ).value == "COMPLETED"
    assert sections[2].find_by_css(
        '.task-list__item__banner'
    ).value == "NOT STARTED"
    assert sections[3].find_by_css(
        '.task-list__item__banner'
    ).value == "NOT STARTED"
    assert sections[4].find_by_css(
        '.task-list__item__banner'
    ).value == "NOT STARTED"
