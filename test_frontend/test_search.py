...
def test_search_for_a_barrier(page):
    ...
    expect(page.get_by_role("heading", name="Market access barriers Search")).to_be_visible()
...
def test_saved_search(page, create_test_barrier):
    ...
    # back to search page
    assert page.locator(".filter-results-title .filter-results-header__row-item").inner_text() == "1"
    expect(page.get_by_role("heading", name="1barrier")).to_be_visible()
